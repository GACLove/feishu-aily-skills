...
label: 仪器数据转换
description: 将实验室仪器输出文件（PDF、CSV、Excel、TXT）转换为 Allotrope Simple Model (ASM) JSON 格式或扁平化 2D CSV。适用于科学家将仪器数据标准化以导入 LIMS 系统、数据湖或下游分析。支持自动检测仪器类型，输出包括完整 ASM JSON、扁平化 CSV 和可导出的 Python 代码。
---
