...
> **重要：`search_by_mql` 的 `mql` 参数必须是完整的 SQL 查询语句**
...
> 以下示例优先使用字段 key。字段 key 从 `list_workitem_field_config` 返回值获取；找不到 key 时可用中文字段名兜底。
...
```sql
-- __需求排期_结束时间 为排期子字段，名称因空间配置而异，需从 list_workitem_field_config 确认
